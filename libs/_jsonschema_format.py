...
def _checks_drafts(both=None, draft3=None, draft4=None, raises=()):
    ...
    def wrap(func):
        if draft3:
            ...
            func = IFormatChecker.cls_checks(draft3, raises)(func)
        if draft4:
            _draft_checkers["draft4"].append(draft4)
            func = IFormatChecker.cls_checks(draft4, raises)(func)
        return func
    return wrap
...
@_checks_drafts("timestamp")
def is_timestamp(instance):
    now = str(time.time()).split(".")[0]

    #check time, if it is not in (now-1hour, now+5minutes), return false
    if int(instance) > int(now) + 300 or int(instance) < int(now)-3600:
        raise FormatError("Wrong time: " + str(instance))
        # return False
    else:
        return True
...
def is_ipv4(instance):
    ...
    if not isinstance(instance, str_types):
        raise FormatError(
                "%r is not a ip-address" % (instance)
            )
...
if hasattr(socket, "inet_pton"):
    ...
    def is_ipv6(instance):
        if not isinstance(instance, str_types):
            raise FormatError(
                "%r is not a ipv6 format" % (instance)
            )
...
def is_host_name(instance):
    if not isinstance(instance, str_types):
        raise FormatError(
                "%r is not a host-name format" % (instance)
            )
        # return True
...
try:
    ...
    def is_uri(instance):
        if not isinstance(instance, str_types):
            raise FormatError(
                "%r is not a uri format" % (instance)
            )
...
def is_regex(instance):
    if not isinstance(instance, str_types):
        # return True
        raise FormatError(
                "%r is not a regrex format" % (instance)
            )
...
def is_date(instance):
    if not isinstance(instance, str_types):
        raise FormatError(
                "%r is not a date format" % (instance)
            )
        # return True
...
draft3_format_checker = IFormatChecker(_draft_checkers["draft3"])
draft4_format_checker = IFormatChecker(_draft_checkers["draft4"])
